export async function onRequestPost(context) {
  ...
  try {
    ...
    if (existing) {
      return new Response(JSON.stringify({ success: false, message: "Username sudah dipakai!" }), { status: 409 });
    }

    // Masukkan ke database
    await env.DB.prepare("INSERT INTO users (id, username, password) VALUES (?, ?, ?)").bind(userId, username, hashedPassword).run();

    return new Response(JSON.stringify({ success: true, userId, username }), { headers: { 'Content-Type': 'application/json' } });
  } catch (error) {
